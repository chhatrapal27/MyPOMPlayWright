...
Call log:
  - waiting for locator('#create')
    - locator resolved to <button id="create" type="button" class="btn btn-dark selenium_btn">Generate File</button>
    ...
    - main [ref=e40]:
      - generic [ref=e41]:
        ...
        - generic [ref=e51]:
          ...
          - generic [ref=e53]:
            - text: "Enter Data:"
            - textbox [active] [ref=e54]: This is a test file for download
...
```ts
  ...
  7  |  console.log(await page.locator('#create').isEnabled());
  8  |  //await page.locator('#create').click();
  9  |  //await expect(page.locator('#link-to-download')).toBeEnabled();
  10 |  //await expect(page.locator('#link-to-download')).toBeVisible();
  11 |  
  12 |      console.log('Create enabled:', await page.locator('#create').isEnabled());
  13 | 
> 14 |     await page.locator('#create').click();
     |                                   ^ Error: locator.click: Test timeout of 30000ms exceeded.
  15 | 
  16 |     console.log('Download link count:', await page.locator('#link-to-download').count());
  17 | 
  18 |     console.log('Download link visible:', await page.locator('#link-to-download').isVisible());
  19 | 
  20 |     console.log('Download link href:', await page.locator('#link-to-download').getAttribute('href'));
  21 |  const downLoadPromise = page.waitForEvent('download');
  22 |  await page.locator('#link-to-download').click();
  23 | 
  24 |  const dwt = await downLoadPromise;
  25 | 
  26 |  const fileName = dwt.suggestedFilename();
  27 |  console.log('Downloaded file: ' + fileName);
  28 |  const filePath = `downloads/${fileName}`;
  29 |  await dwt.saveAs(filePath);
  30 | })
```